...
if ($Uninstall) {
  ...
  @(
    (Join-Path $desktop 'Codex Image Skin.lnk'),
    (Join-Path $desktop 'Codex Image Skin - Restore.lnk'),
    (Join-Path $startMenu 'Codex Image Skin.lnk')
  ) | ForEach-Object { Remove-Item -LiteralPath $_ -Force -ErrorAction SilentlyContinue }
}

if ($RestoreBaseTheme) {
  $backup = Join-Path $StateRoot 'config.before-image-skin.toml'
...
Write-Host 'The live Image Skin was removed.'
