...
foreach ($asset in @('assets\theme-reference.png', 'assets\theme.generated.css', 'assets\theme.generated.json')) {
  $path = Join-Path $SkillRoot $asset
  if (-not (Test-Path -LiteralPath $path)) { throw "Theme asset not found: $path. Run generate-image-theme.ps1 first." }
}
...
if (-not $debugReady -and -not $ProfilePath -and $mainProcesses.Count -gt 0) {
  if (-not $RestartExisting) {
    throw "Codex is already running without image-skin debugging on port $Port. Close Codex or rerun with -RestartExisting."
  }
...
if (-not $verified) { throw 'Image skin launched but verification failed. See injector logs.' }
Write-Host "Codex Image Skin is active on port $Port."
